...
class TestWeightDrift:
    ...
    def test_calculate_drift_overweight(self):
        ...
        drift = calculate_weight_drift(current_weight=0.12, target_weight=0.10)
    ...
    def test_calculate_drift_underweight(self):
        ...
        drift = calculate_weight_drift(current_weight=0.06, target_weight=0.08)
        assert drift == pytest.approx(-0.02, abs=0.0001)

    def test_calculate_drift_exact(self):
        """Test drift when current equals target."""
        drift = calculate_weight_drift(current_weight=0.10, target_weight=0.10)
        assert drift == 0.0
...
class TestSectorWeights:
    ...
    def test_calculate_sector_weights(self):
        ...
        sector_weights = calculate_sector_weights(position_weights, SECTOR_MAPPING)
...
class TestImpliedWeights:
    ...
    def test_calculate_implied_weights_no_change(self):
        ...
        current_weights = {"AAPL": 0.10, "NVDA": 0.08, "SPY": 0.15}

        price_changes = {"AAPL": 1.0, "NVDA": 1.0, "SPY": 1.0}
    ...
    def test_calculate_implied_weights_with_gains(self):
        ...
        current_weights = {"AAPL": 0.10, "NVDA": 0.10}

        price_changes = {"AAPL": 1.0, "NVDA": 1.2}
